...
/*
 * Compare given string UUID with the GpuBlacklist registry parameter string and
 * return whether the UUID is in the GPU blacklist
 */
NvBool nv_is_uuid_in_gpu_blacklist(const NvU8 *uuid)
{
    char *list;
    char *ptr;
    char *token;

    if (NVreg_GpuBlacklist == NULL)
        return NV_FALSE;

    if ((list = rm_remove_spaces(NVreg_GpuBlacklist)) == NULL)
        return NV_FALSE;
