...
__global__ void
srad_cuda_1(float *E_C, float *W_C, float *N_C, float *S_C,
	    float *J_cuda, float *C_cuda, 
	    long cols, long rows, float q0sqr) 
{
	//block id
	long	bx = (long)blockIdx.x;
	long	by = (long)blockIdx.y;
	...
	long	index_n = cols * BLOCK_SIZE * by + BLOCK_SIZE * bx + tx - cols;
	long	index_s = cols * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * BLOCK_SIZE + tx;
	long	index_w = cols * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * ty - 1;
	long	index_e = cols * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * ty + BLOCK_SIZE;
	...
	north[ty][tx] = index_n >= 0 && index_n < cols * rows ? J_cuda[index_n]: 0;
	south[ty][tx] = index_s >= 0 && index_s < cols * rows ? J_cuda[index_s]: 0;
	if (by == 0) {
	...
	west[ty][tx] = index_w >= 0 && index_w < cols * rows ? J_cuda[index_w]: 0;
	east[ty][tx] = index_e >= 0 && index_e < cols * rows ? J_cuda[index_e]: 0;
...
__global__ void
srad_cuda_2(float *E_C, float *W_C, float *N_C, float *S_C,	
	    float *J_cuda, float *C_cuda,
	    long cols, long rows, float lambda, float q0sqr) 
{
	//block id
	long	bx = blockIdx.x;
	long	by = blockIdx.y;
