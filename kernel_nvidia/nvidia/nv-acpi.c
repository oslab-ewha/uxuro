    ...
    if (!strcmp(info->device_class, ACPI_VIDEO_CLASS)) {
        if (info->type == ACPI_VIDEO_NOTIFY_PROBE) {
            if (nv_kmem_cache_alloc_stack(&sp) == 0) {
                /*
                 * If GPU is in GC6 state, this function wakes up the GPU
                 * and notifies the client about display hot-plug/unplug.
                 */
                rm_handle_display_change(sp, nv);
            ...
            /*
             * NOTIFY_BAD tells acpi-video to avoid generating
             * keypresses for this event.
             */
            return NOTIFY_BAD;
        }
    }
