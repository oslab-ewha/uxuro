...
// Flush and/or invalidate a range of memory 
// start, end are the virtual addresses
...
)
{
    ...
    err_no = ipmi_set_my_address(p_priv->p_user, 0, myAddr);
...
{
#if defined(O_TMPFILE) && defined(NV_KERNEL_WRITE_PRESENT)
    struct file *file;
...
{
#if defined(O_TMPFILE) && defined(NV_KERNEL_READ_PRESENT)
    loff_t f_pos = offset;
    ssize_t num_read;
    int num_retries = NV_MAX_NUM_FILE_IO_RETRIES;
